<style>
</style>

<div class="container-fluid">
  <div class="row center-align" id="home-top-button">
    <div class="btn-group" data-toggle="buttons">
      <label class="btn btn-primary active" style="border-bottom-left-radius: 16px;
      border-top-left-radius: 16px;">
      <input type="radio" name="options" id="option1" autocomplete="off" checked> 해경
    </label>
    <label class="btn btn-primary" style="border-bottom-right-radius: 16px;
    border-top-right-radius: 16px;">
    <input type="radio" name="options" id="option2" autocomplete="off"> 동춘
  </label>
</div>
</div>
<div class="row">
  <div class="col-xs-12">
    <div class="card_holder">
      <div class="card" style="height:100px;">
        <div style="position: absolute;
        width: 40px;
        height: 40px;
        background: white;
        border: 1px solid #46292b;
        border-radius: 50%;
        z-index: 2;
        left: 50%;
        top: 50%;
        margin-left: -20px;
        margin-top: -20px;
        text-align: center;">
        <span style="display: block;
        font-size: 10px;
        margin-top: 6px;">전체</span>
        <span style="display: block; font-size: 10px;">보기</span>
      </div>
      <div class="row" style="height:100%;">
        <div class="col-xs-6" style="height:100%;">
          <div>
            <span style="font-size: 8px;margin-right: 4px;">TO</span>
            <span style="font-size: 13px; ">해경 / haekyung</span>
          </div>
          <div style="position:absolute;bottom:0;width:100%;text-align:center;margin-left: -15px;">
            <span style="font-size: 25px; font-weight: 300;">12:15</span>
          </div>
        </div>
        <div class="col-xs-6" style="border-left:1px solid #fcd90d; height:100%;">
          <div style="text-align:right;">
            <span style="font-size: 8px;margin-right: 4px;">FROM</span>
            <span style="font-size: 13px; ">해경</span>
          </div>
          <div style="position:absolute;bottom:0;width:100%;text-align:center;margin-left: -15px;">
            <span style="font-size: 25px; font-weight: 300;">12:15</span>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>
</div>
<div class="row" style="margin-top:20px;">
  <div class="col-xs-12 center-align">
    <span style="font-size: 13px;
    margin-right: 5px;">일반 버스 실시간 정보</span><i class="glyphicon glyphicon-repeat"></i>
  </div>
</div>
<div class="row" style="margin-top:20px;">
  <div class="col-xs-12">
    <div class="card_holder">
      <div class="card">

      </div>
    </div>
  </div>
</div>
</div>
</div>
</div>
</div>
</div>
