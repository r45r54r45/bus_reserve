...
<div id="help"  class="col-xs-12  height-100 thecenter color border-all">
	<div>
		<div style="margin-top:10px;color:white;" class="small-font">made by cocoding</div>
...
function getFood(){
	$.get("/data/food", function( data ) {
		...
		for (var i = 0; i < one.length; i++) {
			$("#one_"+i+"_1").text(one[i][0]); //음식 1
			$("#one_"+i+"_2").text(one[i][1]); //음식 2
		}
		for (var i = 0; i < two.length; i++) {
			$("#two_"+i+"_1").text(two[i][0]); //음식 1
			$("#two_"+i+"_2").text(two[i][1]); //음식 2
		}
	}, "json" );
}
function help(){
